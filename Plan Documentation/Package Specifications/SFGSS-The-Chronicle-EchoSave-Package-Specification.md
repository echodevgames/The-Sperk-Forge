...
| ESV-D-041 | M5-05 closes two pre-Laboratory MVP gaps before scene/sample proof: explicit unknown-payload prune is a plan-bound exact-ID destructive operation that publishes a new immutable generation, while `catalog.cache.json` is derived acceleration only and must be safely rebuildable from heads/manifests | Approved | LAB-016 requires explicit prune and LAB-029 requires catalog-cache rebuild. Activating the Laboratory before those capabilities exist would knowingly create an acceptance matrix that cannot close truthfully | M5-05 may add public/internal prune plan/result composition, source-bound confirmation, derived cache schema/read/write/rebuild coordination, Editor cache Preview/Rebuild tooling, and focused tests. It may not rewrite committed generations, prune claimed/known entries, treat cache as authority, add quarantine/incomplete cleanup, restore trash, permanently erase, activate automatic recovery, generic queues, automatic autosave triggers, permission-provider wiring, or add Save Laboratory scene/sample content. The full Laboratory becomes M5-06 | No |
| ESV-D-042 | The Chronicle Save Laboratory is a minimal isolated direct-scene engineering harness, not a polished consumer save-menu example. It may use one tiny deterministic Sperk-themed participant state, crude sample-owned controls/readouts, deterministic evidence fixtures, and grouped LAB-001 through LAB-032 interactions. Rich recognizable save-menu examples belong to the later Chronicle Reference Showcase after Looking Glass and preferably Resonance are available | Approved | Chronicle runtime behavior is already heavily automated-tested. Remaining Laboratory value is human-visible integration proof; polished UI now would duplicate future Looking Glass work and risk accidental UI architecture inside EchoSave | M5-06 may add sample-only direct-scene harness code/assets, minimal evidence controls, one small proof participant, deterministic fixtures, and LAB evidence logging. It may not add Looking Glass/Resonance dependency, reusable player-facing save-menu framework, multi-theme showcase templates, fake gameplay systems, or change runtime APIs merely for presentation convenience | No |
...
R1 completion did not complete M4. R2 slot-policy runtime configuration was the next gate at R1 closeout and is now complete at `8a8e7e7`. R3 package-document migration and final registry/document reconciliation remain required. M5 remains locked.

### 28.28 ESV-M4-R2 slot-policy runtime configuration and CAP-002 reconciliation
...
R2 began from the historical technical capacity `64` and replaced that hidden runtime authority with project-owned, validated, schema-versioned configuration. The legacy internal constant remains only as a source-compatibility alias to schema-1 behavior and does not drive schema-2 capacity.
...
R2 completed CAP-002 with schema-2 project-owned slot-policy configuration, all four approved policy modes, one immutable service-session effective capacity shared by create and duplicate, and non-mutating schema-1 compatibility at historical capacity 64. ESV-T-015 through ESV-T-018 are complete.
...
- **24** net new focused R3 migration tests;
- committed implementation/test scope **17 files**, `3359` insertions, `31` deletions;
- no bounded compile/test hotfix was required after implementation apply.

Completed CAP-014 package-document truth:
- exact-current package documents bypass migration;
- historical package documents can traverse deterministic contiguous exact-version package-owned chains in memory;
- migration reaches the exact current package version before existing strict current DTO validation;
- missing, ambiguous, invalid, failed, throwing, overshooting, and unsupported-newer paths fail closed;
- source generation bytes and `head.json` are never rewritten or republished merely because a read required migration;
- package-document migration remains separate from participant migration and precedes participant preparation/migration;
- current-generation loading, catalog scanning, and recovery candidate verification consume the migration-aware package-document read seam where versioned package documents are read;
- raw stored bytes remain the basis for payload integrity checks;
- current production envelope, manifest, payload, and head-pointer versions remain `1.0.0`;
- the production package-document migration registry remains empty until a real package-format step exists.

R3 closes audit blocker **A-04 / CAP-014**. Registry rows **ESV-T-067**, **ESV-T-068**, **ESV-T-069**, **ESV-T-072**, and **ESV-T-073** now have retained R3 evidence.

The final applicable 100-case registry/document evidence reconciliation is next but is **not activated by R3 closeout**. M4 remains open and M5 remains locked.

### 28.30 ESV-M4-R4 final 100-case registry, documentation evidence reconciliation, and M4 closeout
...
**Carried focused regression floor:** **660 / 660**.

**Authority decision:** ESV-D-036.

#### R4 purpose

R4 is the final M4 evidence-and-documentation reconciliation gate. It does not add another Chronicle runtime capability.

Its job is to determine, from retained repository evidence, exactly which of the 100 specification registry rows have already been proven by completed M1-M4 work, which rows remain deliberately later work, and whether any M4-applicable row is still missing direct proof.

The governing rule is:

> Reconcile evidence honestly. Do not convert “planned someday” into “complete now,” and do not leave already-proven M4 behavior falsely marked Planned.

#### Registry reconciliation rules

Every row from **ESV-T-001 through ESV-T-100** must be reviewed individually.

For each row, R4 must record:

- current registry status;
- capability/scenario intent;
- M4 applicability;
- retained evidence source when one exists;
- final reconciled status/disposition;
- issue/blocker note when evidence is insufficient.

A row may be marked **Complete** only when retained evidence directly proves the row's expected result.

Acceptable retained evidence includes:

- a named automated Chronicle test or test class that is still present and green;
- a checkpoint implementation/closeout record whose executed test gate directly proves the scenario;
- a retained manual evidence record when the scenario is inherently manual and that record is authoritative.

Checkpoint completion by itself is not permission to mark unrelated registry rows Complete.

#### Later-work disposition

The 100-case registry spans more than M4 runtime implementation. Rows whose authoritative scope belongs to later work remain not complete.

Examples include, where applicable:

- M5 Editor tooling, Setup, Validator, Browser, Simulator, and Save Laboratory work;
- direct-scene/Laboratory scenarios not yet executed;
- clean-project reproduction;
- external tarball/Git/package installation qualification;
- platform qualification;
- performance/stress measurements not yet executed;
- integration/adoption work with First Light, Looking Glass, Passage, or existing projects;
- release qualification/private beta evidence.

Such rows must remain **Planned**, **Deferred**, or otherwise explicitly not-complete according to the reconciled documentation. They do not become Complete merely because M4 is ready to close.

#### M4-applicable gap rule

If a registry row is determined to be M4-applicable and the repository does not contain retained proof for it:

1. do not infer completion;
2. do not write a documentation-only claim;
3. record the row as an unresolved M4 reconciliation blocker;
4. stop M4 closeout;
5. create a separate bounded repair checkpoint before changing runtime or test code.

R4 itself authorizes **no runtime or test-code changes**.

This keeps the final reconciliation from smuggling new implementation into what is supposed to be an evidence audit.

#### Documentation parity scope

R4 must reconcile stale current-state claims across the repository, including at minimum:

- this Chronicle package specification and the 100-case registry;
- `Plan Documentation/Current Notes.md`;
- package `Documentation~/Developer/Current Notes.md`;
- `ESV-M4_Chronicle_Milestone_Reconciliation_Audit.md`;
- `Suite_Health_Check_and_Remaining_Documentation.md`;
- package `README.md`;
- package `CHANGELOG.md`;
- package documentation index / navigation pages that state current capability or milestone status;
- R1/R2/R3/final checkpoint records where current-state wording requires correction;
- new-conversation handoff truth.

Historical evidence must remain historical. R4 should repair current-state contradictions without rewriting valid historical checkpoint records into a fictional single narrative.

#### Test execution rule

The incoming focused Chronicle Editor floor is **660 / 660 passed, 0 failed**.

Before M4 can close:

1. the registry/document reconciliation must be complete;
2. the focused `EchoDevGames.EchoSave.Tests.Editor` suite must be rerun;
3. the actual discovered closing total must be recorded;
4. failures must be resolved before closeout;
5. the final evidence must not claim a count that was not executed.

Because R4 authorizes no new test code, the expected discovery count is normally unchanged. If the discovered count differs, record the actual result and investigate rather than forcing the historical number.

#### M4 close rule

R4 may declare **M4 complete** only if all of the following are true:

- A-01 through A-04 remain resolved;
- every ESV-T-001 through ESV-T-100 row has an explicit reconciled disposition;
- every M4-applicable row is supported by retained evidence;
- later milestone/release rows remain honestly not complete;
- package/suite documentation agrees on the current runtime boundary;
- no undocumented M4 runtime blocker remains;
- the final focused Chronicle Editor suite is green at the actual closing total;
- the working tree is clean after the closeout commit.

If any condition fails, M4 remains open.

#### M5 lock

R4 activation does not activate M5.

Even a green R4 evidence run does not activate M5 until the **committed R4 closeout** explicitly records M4 complete.

M5 must begin, if approved, through its own separately bounded activation commit.

#### R4 expected artifact sequence

1. **Activation commit**
   - authority v1.41.0 / ESV-D-036;
   - R4 checkpoint plan;
   - no runtime changes.
2. **Evidence reconciliation work**
   - row-by-row 100-case mapping;
   - stale README/CHANGELOG/index/current-state repair;
   - no runtime/test-code changes unless a separate blocker checkpoint is activated.
3. **Final focused Unity gate**
   - rerun the Chronicle Editor suite;
   - record the actual total.
4. **R4 / M4 closeout commit**
   - final registry state;
   - final M4 capability/evidence matrix;
   - final documentation parity;
   - M4 complete only if every close rule passes.
5. **Later separate M5 activation**
   - never implied by R4 activation or test success.
...
Treat The Chronicle (EchoSave) Package Specification v1.41.0 as the authority
...
Before writing more Chronicle runtime code or activating M5:
1. Treat `e3d7a2e` as the clean R4 planning baseline and `c6ba1ad` as the retained R3 implementation baseline.
2. Preserve the **660 / 660** focused Chronicle regression floor.
3. Execute ESV-M4-R4 as a documentation/evidence reconciliation only.
4. Reconcile ESV-T-001 through ESV-T-100 individually; mark Complete only from retained direct evidence and leave later M5/release/adoption rows explicitly not complete.
5. If an M4-applicable row lacks proof, stop closeout and create a separate bounded repair checkpoint before changing runtime or tests.
6. Synchronize stale README, CHANGELOG, package index, Current Notes, Suite Health, audit, specification, and handoff truth.
7. Rerun the focused Chronicle suite and record the actual closing total.
8. Keep M4 open and M5 locked until the committed R4 closeout proves every M4 close rule.
```
...
| Field | Current value |
|---|---|
| Package version | Runtime package `0.1.0`; Specification v1.41.0 |
| Completed checkpoint | ESV-M4-R3 - Package-Document Migration and CAP-014 Reconciliation |
| Implementation commit | `c6ba1ad` |
| Tests passed | Final ESV-M4-R3 focused Chronicle Editor gate `660 / 660`; prior `636 / 636` regression floor preserved; Unity compile/import green |
| Tests failed | Final ESV-M4-R3 gate: `0` |
| Known issues | Runtime audit blockers A-01 through A-04 are resolved; ESV-M4-R4 is active to reconcile all 100 registry rows/document evidence before M4 close |
| Decisions added | ESV-D-001 through ESV-D-036; ESV-D-036 defines honest row-by-row final registry evidence classification and forbids documentation-only invention of missing M4 proof |
| Active learning checkpoint | PKG-LEARN-009 complete |
| Implementation permission | ESV-M4-R4 is active for documentation/evidence reconciliation and final test rerun only. Runtime/test-code changes are not authorized; unresolved M4-applicable gaps require a separate repair checkpoint. M4 remains open and M5 locked. |
...
### 28.34 ESV-M5-02 Full Setup/Configuration Authoring and Safe Repair Preview activation
...
**Carried focused Chronicle regression floor:** **697 / 697 passed, 0 failed**.

**Authority decision:** ESV-D-038.

M5-02 extends the M5-01 safety shell to editing/upgrading real project-owned Chronicle configuration and selected setup references.

M5-02 owns:
1. explicit schema-3 `EchoSaveConfiguration` authority where needed to make the approved configuration model truthful;
2. deterministic schema-1/schema-2 compatibility mappings with **no runtime asset mutation**;
3. explicit Editor upgrade preview for older selected configuration assets;
4. authoring for existing slot policy plus retention/provider/limit/recovery/template configuration only where the consuming runtime/tooling path exists;
5. optional project-owned fixed-slot template authoring without runtime auto-provisioning or path authority;
6. optional project-owned root prefab creation/assignment with preview before creation or selected reference mutation;
7. safe selected configuration/root/reference repair plans showing exact before/after values and affected assets/objects;
8. explicit Apply with Unity Undo/dirty/save semantics and no silent overwrite;
9. expanded Validator coverage for retention/provider/template/schema/repair-preview truth that M5-02 makes representable;
10. focused Editor/runtime configuration tests proving compatibility, deterministic preview, one-shot Apply truth, repeat safety, no-clobber, Undo-safe selected repair, and zero production save-data mutation.

Schema evolution rules:
- schema 3 is the current authoring target after M5-02 implementation;
- schema 2 retains exact M4 slot-policy behavior plus deterministic defaults for newly introduced schema-3 configuration domains;
- schema 1 retains the historical slot-capacity-64 compatibility mapping and the same deterministic defaults;
- unsupported future schemas fail closed;
- runtime never upgrades or rewrites the ScriptableObject;
- Editor upgrade is explicit, Preview-first, target-bound, and revalidates immediately before Apply.

Configuration truth rule:
M5-02 must not create decorative fields. A provider ID, limit, retention bound, recovery preference, or template reference is authorable only if the implementation can validate and consume it truthfully. Unsupported/future values remain blocked rather than silently ignored.

Repair safety rule:
- repairs target only explicitly selected project assets/objects;
- duplicate-root authority is never auto-chosen;
- unsafe paths are never silently normalized into a different durable root;
- existing assets are not replaced by default;
- fixed-slot identity collisions are never regenerated automatically when durable save evidence could exist;
- production save directories/generations/heads/trash are never mutated by Setup repair;
- any destructive or ambiguous repair is preview-only and blocked in M5-02.

M5-02 explicitly does **not** own:
- Save Browser, Generation Inspector, or Migration Graph;
- Failure Simulator, Recovery Planner UI, Test Data Generator, or redacted snapshot export;
- persistent `catalog.cache.json`;
- quarantine/incomplete-generation cleanup, permanent erase, or restore-from-trash;
- direct-scene initializer, Save Laboratory sample scene/UI/sandbox, or LAB-001 through LAB-032 execution;
- scene travel, peer-package bridges, service-locator behavior, or Chronicle-owned/project-wide DDOL;
- package document version changes;
- participant-contract changes;
- weakening immutable generation/head-last/recovery semantics.

Closeout requires:
- implementation commit;
- clean Unity compile;
- focused `EchoDevGames.EchoSave.Tests.Editor` green at the actual discovered total and not below **697 / 697**;
- manual Preview/Apply/repair sanity proof for one created/edited configuration and one safe selected-reference repair;
- package README/CHANGELOG/Index, both Current Notes, Suite Health, checkpoint, and specification reconciliation;
...
### 28.36 ESV-M5-03 Save Browser, Generation Inspector, and Migration Graph activation
...
**Carried focused Chronicle regression floor:** **724 / 724 passed, 0 failed**.

**Authority decision:** ESV-D-039.

M5-03 owns a strictly read-only inspection layer.

#### Save Browser

The Browser may:
- refresh discovered Chronicle slots from real catalog truth;
- show slot identity/display metadata without using display text as path authority;
- show current head/generation state;
- show trash/recoverability/health state already represented by Chronicle;
- show last-known publication/recovery warnings;
- select a slot for deeper inspection.

The Browser may not:
- create, duplicate, delete, restore, recover, rename, or repair a slot;
- change a head;
- mutate save bytes;
...
The Inspector may:
- enumerate immutable committed generations for a selected slot;
- show generation ID, timestamps/ordering truth, head/current relationship, manifest/package-document metadata, integrity/support state, and recovery candidacy;
- inspect package-document versions without mutating them;
- surface unsupported-newer, missing-migration, incomplete/corrupt, or stale-head evidence.

The Inspector may not rewrite generation/package documents or trigger recovery.

#### Migration Graph

The Graph may:
- enumerate registered package-document migration steps;
- show directed source-version → target-version edges;
- show reachable paths to the current package-document version;
- identify gaps, ambiguity, cycles, duplicates, or unsupported-newer states;
- show which participant/package document would require a missing path.

The Graph is descriptive only. It does not run migrations against production data.

#### Read-only runtime seam

If Editor tooling cannot consume existing internal truth safely, M5-03 may add narrow additive runtime models/query services that:
- expose immutable/read-only snapshots;
- return copied DTO collections rather than mutable internal structures;
- do not provide mutation handles;
- preserve current operation/runtime contracts;
- do not add UnityEditor dependencies to Runtime.
...
- Test Data Generator;
- redacted snapshot/support export;
- persistent catalog cache;
- cleanup/quarantine/permanent erase/restore-from-trash;
- direct-scene initializer or Save Laboratory;
- LAB-001 through LAB-032;
- scene travel, peer-package bridges, service-location, or project-wide DDOL;
- package document version changes;
- participant contract changes.
...
- implementation commit;
- clean Unity compile;
- focused Chronicle Editor suite green at actual discovered total and not below **724 / 724**;
- manual Browser refresh/selection proof;
- manual Generation Inspector proof over at least one disposable committed generation;
- manual Migration Graph proof showing registered edges/path or an intentionally empty/no-migration state accurately;
- proof that refresh/selection/inspection performs zero writes;
...
### 28.40 ESV-M5-05 Explicit Unknown-Payload Prune and Derived Catalog Cache/Rebuild Prerequisites activation
...
**Carried focused Chronicle regression floor:** **746 / 746 passed, 0 failed**.

**Authority decision:** ESV-D-041.

#### Sequencing correction

Prior M5 documentation used shorthand that M5-05 would directly activate the Save Laboratory.

That shorthand is superseded because the already-approved Laboratory matrix requires two capabilities that remain absent after M5-04:
- **LAB-016:** explicitly prune one unknown entry;
- **LAB-029:** rebuild `catalog.cache.json` and prove the slot list matches durable manifests.

The corrected bounded sequence is:
- **M5-05:** implement/prove these two prerequisites;
- **M5-06:** activate the full direct-scene Chronicle Save Laboratory and reconcile LAB-001 through LAB-032.

This is a checkpoint-order correction, not a change to package ownership or durability architecture.

#### Explicit unknown-payload prune

Unknown payload preservation remains the default.

M5-05 may add a public/runtime prune workflow only under these rules:

1. Preview/planning is read-only.
2. A plan names exact opaque unknown participant IDs; wildcard/all-unknown pruning is not authorized.
3. The plan binds:
   - package/session authority;
   - selected/target `SaveSlotId`;
   - exact source current generation/provenance;
   - exact requested unknown IDs;
   - bounded expiry/one-use confirmation.
4. Confirmation revalidates the source generation/provenance before any durable write.
5. Every requested ID must still be unknown/unclaimed at confirmation time.
6. Known/current participant IDs may never be removed through the unknown-prune surface.
7. Unnamed unknown entries remain byte-for-byte preserved.
8. Successful prune publishes a **new immutable generation** and then advances head using existing durable publication authority.
9. No committed generation is edited in place.
10. Stale, replayed, expired, ambiguous, or newly-claimed plans fail before publication.
11. Mutation reuses existing root-local operation admission and does not create a generic queue.
12. Result truth distinguishes no-op, rejected, publication failure, head failure, catalog/maintenance follow-up failure, and success.

#### Derived `catalog.cache.json`

`catalog.cache.json` is package-owned derived state, not durable slot authority.

M5-05 cache rules:

1. Durable heads/manifests always win.
2. Missing cache is a normal cold-start condition.
3. Corrupt/parse-failed cache is ignored and rebuilt.
4. Unsupported/incompatible cache schema is ignored/rebuilt rather than blocking save access.
5. Stale cache must be detected from bounded durable provenance/freshness evidence.
6. A valid cache may accelerate catalog startup/listing only after validation.
7. Rebuild derives entries from existing canonical slot discovery/head/current-manifest truth.
8. Cache cannot convert a degraded/corrupt/missing durable slot into a healthy one.
9. Cache write/replace happens only after live catalog truth exists.
10. Cache write failure reports maintenance degradation but cannot fabricate rollback of a valid in-memory catalog or prior save operation.
11. Partial/temp cache files never become sole authority.
12. Cache size/count remains bounded by existing configuration discovery limits.
13. Editor tooling may offer read-only Preview followed by explicit **Rebuild Catalog Cache**.
14. Editor rebuild may replace only the derived cache file and may not alter slot/generation/head/payload documents.

#### Required focused evidence

At minimum:
- prune Preview performs zero writes;
- exact-ID plan carries source/provenance and requested IDs;
- stale source rejects before publication;
- newly claimed ID rejects before publication;
- unnamed unknown entries remain byte-identical;
- named unknown entry is absent only in the newly published generation;
- historical source generation bytes are unchanged;
- no-op/invalid/duplicate requested IDs are deterministic;
- missing cache rebuilds correctly;
- valid cache matches durable rebuild;
- corrupt cache is ignored/rebuilt;
- stale cache is invalidated by durable head/manifest change;
- incompatible cache does not block catalog access;
- cache-write failure leaves truthful live catalog intact;
- explicit Editor rebuild Preview is zero-write;
- explicit rebuild changes only `catalog.cache.json`;
- LAB-016 and LAB-029 prerequisites have direct automated/manual evidence;
- full focused Chronicle Editor suite remains green at actual discovered total and not below **746 / 746**.

#### Explicitly out of scope

M5-05 does not authorize:
- direct-scene Save Laboratory content;
- LAB-001 through LAB-032 execution as a complete matrix;
- production quarantine/incomplete-generation cleanup;
...
### 28.42 ESV-M5-06 Minimal Direct-Scene Save Laboratory activation

**Status:** Active / Authorized.

**Clean planning baseline:** `868b17f`.

**Carried focused Chronicle regression floor:** **753 / 753 passed, 0 failed**.

**Authority decision:** ESV-D-042.

The Save Laboratory answers one question: can a human open one isolated scene, press obvious controls, and visibly verify Chronicle's real runtime outside NUnit?

M5-06 may add one sample-owned participant with a handful of primitive values, a minimal engineering panel, deterministic fixture buttons, and an evidence console. Suggested sample flavor includes `Sperk Level`, `Galactic Rupees`, `Anvil Temperature`, `Has Forbidden Key`, `Reality Damage`, and restrained labels such as `DO NOT PRESS UNLESS REALITY IS BACKED UP` or `THE CHRONICLE REMEMBERS.` Runtime APIs and diagnostics remain neutral.

The primary visual proof should be Save -> mutate values without saving -> Load -> visibly restore the saved values. LAB-001 through LAB-032 may be grouped behind a small number of real controls and deterministic fixtures; the Lab must never print PASS without checking actual Chronicle result/state.
