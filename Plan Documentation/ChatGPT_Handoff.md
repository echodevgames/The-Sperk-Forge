...
11. If `gh` is unavailable inside the assistant workspace, do not ask Jesse to install it on his Windows machine. Use the connected GitHub app's Git-data path already proven for this repository: verify remote ref → create blobs → require local/remote blob SHA equality → create tree from the exact verified parent tree → require tree equality → create commit → update `main` with `force: false` → fetch through Git → require parent/tree/ref equality → synchronize the local branch ref only after equality is proven.
12. Jesse has authorized uploading EUI-M4-03 source/test files, plus subsequent source files he explicitly approves with `go`, to `echodevgames/The-Sperk-Forge`. If a connector safety gate still requests file-specific disclosure approval, explain the exact paths and request the narrow wording; do not misrepresent missing `gh` as a user-side setup failure.
13. The established bounded slice loop is: present one exact slice and exclusions → wait for `go` → verify clean synchronized parent → implement only that slice → static scope/whitespace/GUID review → commit locally → publish non-forced with exact Git-object equality → request full Unity EditMode XML with expected focused/assembly/full totals → inspect the XML → accept green or fix only the first owned failure → present the next slice. Never batch future slices merely to save conversation turns.
