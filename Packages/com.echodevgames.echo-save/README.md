...
**ESV-M5-02 is now active** as the separately bounded full Setup/configuration-authoring and safe-repair-preview slice. It may advance project configuration to schema 3, but only through explicit compatibility-preserving authority: older schema-1/schema-2 assets remain runtime-readable through deterministic defaults and are never silently rewritten; Editor upgrades/repairs must preview exact asset/reference changes first, preserve Undo/backup-safe behavior, and refuse ambiguous or destructive repairs. It still does **not** include broader provider/retention/recovery authoring, Browser/Inspector/Simulator tooling, Laboratory content, automatic/configured recovery fallback, persistent catalog-cache optimization, permanent erase, public restore-from-trash, generic queued multi-operation scheduling, scene travel, peer bridges, or clean-project/release qualification.
