...
**ESV-M5-04 is complete** at implementation commit `577dc01`. Chronicle now includes sandbox-only Failure Simulator and bounded deterministic Test Data tooling, a preview-only Recovery Planner over existing immutable recovery-plan authority, and an explicit payload-free Redacted Snapshot Exporter. The sandbox guard refuses equality/nesting/containment collisions with production; simulator mutation is exact-target and preview-first; generator cleanup is ownership-verified; support output hashes root/slot/generation identity and omits participant payload contents/full local paths. The closing focused Chronicle Editor gate passed **746 / 746**, with **0 failed**.

**ESV-M5-05 is now active** as the Laboratory-prerequisite runtime/tooling slice: explicit plan-bound unknown-payload prune and a derived rebuildable `catalog.cache.json`. Unknown prune is explicit-ID-only, source-bound, immutable-generation publication; it never rewrites prior generations and never deletes known/claimed participant data. The catalog cache is acceleration only, never authority: missing/corrupt/stale cache falls back to bounded manifest/head rebuild, and cache-write failure cannot fabricate catalog/save failure. A manual/Editor cache rebuild path is authorized for LAB-029 evidence. The full direct-scene Chronicle Save Laboratory moves to M5-06 and remains inactive during M5-05.



It still does **not** include automatic recovery fallback, Failure Simulator/Recovery Planner/Test Data/support export, persistent catalog-cache optimization, cleanup/quarantine, permanent erase, public restore-from-trash, direct-scene Laboratory content, LAB-001 through LAB-032 execution, scene travel, peer bridges, or clean-project/release qualification. **M5 remains open and M5-05 is not active.**
